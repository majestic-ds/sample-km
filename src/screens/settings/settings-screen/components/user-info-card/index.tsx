...
const UserInfoCard = ({user}:Props) => {
  return (
    <View style={{
      ...
      <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10, textAlign:'center' }}>User Information</Text>
      <View>
        <Text>ID: {user.id}</Text>
        <Text>Incharge ID: {user.incharge_id || 'N/A'}</Text>
        <Text>Name: {`${user.first_name} ${user.middle_name} ${user.last_name}`}</Text>
